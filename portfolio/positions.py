...
def get_portfolio_summary(username: str, ticker: str, current_price: float) -> dict | None:
    """
    Agrège tous les lots d'un ticker et calcule la P&L.
    Retourne None si aucune position.
    """
    lots = get_positions(username, ticker)
    if not lots:
        return None

    total_shares  = sum(float(l["quantite"])   for l in lots)
    total_investi = sum(float(l["quantite"]) * float(l["prix_achat"]) for l in lots)
    cout_moyen    = total_investi / total_shares if total_shares else 0
    valeur_actuelle = total_shares * (current_price or 0)
    pnl_euros     = valeur_actuelle - total_investi
...
def add_position(username: str, ticker: str, company: str,
                 ...
                 quantite: float, currency: str = "USD", notes: str = "") -> dict:
    """Insère un lot d'achat. Retourne la ligne créée."""
    ticker = ticker.upper()
    row = {
        "username":   username,
        "ticker":     ticker,
